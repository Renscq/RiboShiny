...
⚠️ For genes with polyploidy and multiple transcripts in a species, it is generally recommended to use representative transcripts for subsequent analysis (`longest`). This is because current alignment software often cannot accurately align true transcripts using short RPFs.
...
⚠️ If you are using a web-based program, you can contact the author for installation assistance. OR you can use the `gson` file for 
...
RiboShiny supports both OrgDb and gson formats, for more information about the great gson format, see home page of Prof. Guangchuang Yu. <https://guangchuangyu.r-universe.dev/gson>
...
⚠️ The longest transcript is recommended here.
...
⚠️ Ribo-seq sequencing data typically exhibits a major peak around 27-30 nucleotides, corresponding to the structural size of the ribosome. 
⚠️ If enzymatic digestion issues occur, results such as excessively long or short fragments, or the absence of a peak, may arise. In such cases, it is necessary to examine the 3-nucleotide (3nt) periodicity of the RPFs.
...
⚠️ RNase and MNase have significantly different preferences for RPFs digestion.
...
⚠️ RSBM usually represents the highest frame of the p-site offset.
...
⚠️ The results of SSCBM can only represent translation near the start and stop codons; further analysis is needed for overall translation elongation. 
...
⚠️ Algorithm predictions are not always completely accurate and often rely on data quality. When data quality is low, it is necessary to examine the results of the raw data to manually correct the offset length.
...
⚠️ Generally, it is recommended to proceed with subsequent analysis, especially codon-level analysis, with data exhibiting periodicity greater than 55%. If only gene-level analysis is required, the quality of 3-nucleotide periodicity is not as critical.
...
⚠️ For leadered transcripts, [-30, 120] nt around the TIS/TTS is typically used.
⚠️ For leaderless transcripts, [-9, 120] nt around the TIS/TTS is typically used.
...
⚠️ Sequencing bias may exist between different samples. Therefore, it is recommended to use the `scaled` option to normalize the data, allowing for comparisons of multiple samples at the same level.
...
⚠️ This step requires importing the sample design file in step0.
⚠️ If batch effects exist, they need to be corrected using RUVSeq for quantification and translation efficiency (TE) calculation.
...
⚠️ It is generally recommended to use the results from RiboParser and the representative transcript for each gene.
⚠️ You can adjust the parameters in RiboParser to quantify the CDS region, including excluding reads that show poor stability around the start and stop codons.
...
⚠️ We use normalized data to create a heatmap, which can be generated from either RSEM's FPKM data or data normalized with RUVSeq.
...
⚠️ We use normalized data to create a heatmap, which can be generated from either RiboParser/RSEM's RPKM data or data normalized with RUVSeq.
...
⚠️ We use the previously calculated translation efficiency data to compute and visualize sample reproducibility.
⚠️ It is recommended to use all the longest representative transcripts here.
⚠️ Given that the formula for calculating TE involves two different sets of data, low-abundance genes are prone to significant fluctuations. Therefore, it is recommended to use genes with RNA-seq and Ribo-seq quantification greater than 5.
...
⚠️ We use normalized data to visualize the PCA results, with RUVSeq normalized data as an example.
⚠️ All the longest representative transcripts are recommended here.
⚠️ Genes with RPM > 5 are recommended for analysis.
...
⚠️ We perform PCA using the translation efficiency calculated from RNA-seq and Ribo-seq data.
⚠️ All the longest representative transcripts are recommended here.
⚠️ Genes with RNA-seq/Ribo-seq RPM > 5 are recommended for analysis.
...
This step is used for differential analysis of RNA-seq or Ribo-seq data, here, we use RNA-seq as an example.

⚠️ We use edgeR for differential analysis, with raw counts as input data; 
⚠️ RUVSeq can be used to romove the unwanted variation like batch effect.
⚠️ Genes with RNA-seq/Ribo-seq RPM > 5 are recommended for analysis.
...
This step is used for differential analysis of RNA-seq or Ribo-seq data, here, we use Ribo-seq as an example.

⚠️ We use DESeq2 for differential analysis, with raw counts as input data; 
⚠️ RUVSeq can be used to romove the unwanted variation like batch effect.
⚠️ Genes with RNA-seq/Ribo-seq RPM > 5 are recommended for analysis.
...
⚠️ We use DeltaTE for the differential analysis. Since the analysis of translation efficiency requires consideration of both RNA-seq and Ribo-seq expression levels, you need to import two files simultaneously.
⚠️ Genes with RNA-seq/Ribo-seq RPM > 5 are recommended for analysis.
...
⚠️ We use the output results from edgeR, DESeq2, or DeltaTE for visualization. 
⚠️ Here, we will use the results from DeltaTE as an example, and you can specify the input file by sheet name, which includes the differential analysis results for TE, RNA-seq, and Ribo-seq.
⚠️ Genes can be labeled by their ids, which are separated by commas.
⚠️ The results can be shown by Volcano plot and MAplot respectively.
⚠️ FoldChange is recommended to be set to ±2, which can be reduced to ±1.5 according to different projects. A lower value is not recommended.
...
This step is for analyzing the coordinated changes between RNA-seq and Ribo-seq. Here, we will use the results from DeltaTE as an example.

⚠️ We will visualize the results from the differential analysis of RNA-seq and Ribo-seq, requiring the simultaneous input of both sets of results. 
⚠️ Here, we will use the results from DeltaTE as an example, and you can specify the input file by sheet name, which includes the differential analysis results for TE, RNA-seq, and Ribo-seq.
⚠️ Genes can be labeled by their ids, which are separated by commas.
⚠️ The NS gene can be removed to focus on the important differential genes.
⚠️ FoldChange is recommended to be set to ±2, which can be reduced to ±1.5 according to different projects. A lower value is not recommended.
...
In contrast to the previous step, here we classify genes based on different trends of change using the DeltaTE method to identify genes that are regulated at the translation level.
...
⚠️ A small tool for analyzing the intersection and aggregation of genes currently supports only four samples.
...
⚠️ Be careful not to have duplicate ids when renaming each file.
...
⚠️ Please note that the gene names in the differential gene table must correspond to the names in the ordgb database.
⚠️ Use the 'simplify' method with caution, as it consumes a significant amount of time.
⚠️ It is recommended to use a p-value < 0.05. If no enrichment results are obtained, you can adjust this parameter for further investigation.
...
⚠️ Please note that the gene names in the differential gene table must correspond to the names in the ordgb database.
⚠️ Pay attention to the grouping information during differential analysis, as it may lead to opposite enrichment results, causing errors in data interpretation.
⚠️ It is recommended to use a p-value < 0.05. If no enrichment results are obtained, you can adjust this parameter for further investigation.
...
⚠️ Please note that the gene names in the differential gene table must correspond to the names in the KEGG database.
⚠️ You can use internal KEGG database or a localized gson file for enrichment.
...
⚠️ All the longest representative transcripts are recommended here.
⚠️ You can filter the input gene set and retain highly expressed genes for calculation.
...
⚠️ Here we use the output file from RiboParser, which is similar but different from bedgraph.
⚠️ The results of RNA-seq and Ribo-seq can be exported separately.
⚠️ Note that the ordinate is adjusted to accommodate different abundance of genes.
...
⚠️ Note that the interpretation of the pausing score on E/P/A-site are different.
⚠️ For accurate pausing score calculations, it is crucial that the data exhibits a periodicity exceeding 60%; otherwise, interpretation of the results may be prone to errors.
⚠️ Sequencing undersaturation can introduce fluctuations in the calculated results. Therefore, it is highly recommended to first verify that the sequencing saturation and coverage are adequate.
...
⚠️ Note that the interpretation of the codon occupancy on E/P/A-site are different.
⚠️ For accurate pausing score calculations, it is crucial that the data exhibits a periodicity exceeding 60%; otherwise, interpretation of the results may be prone to errors.
⚠️ Sequencing undersaturation can introduce fluctuations in the calculated results. Therefore, it is highly recommended to first verify that the sequencing saturation and coverage are adequate.
...
⚠️ Ensure proper data fitting by carefully assigning distinct groups and samples.
...
⚠️ Note that the calculation in RiboParser is divided into different frames.
⚠️ Alternatively, you can merge the data from frame0, frame1, and frame2 longitudinally to create a combined plot.
...
⚠️ Ensure proper data calculation by carefully assigning distinct groups and samples.
...
⚠️ This method utilizes files generated by the `serp_peak` function within RiboParser.
⚠️ `bound` indicates the protein binding region
⚠️ `edge` indicates the blurry area on the `bound` flanks
⚠️ `enrich` indicates the un-binding region
...
⚠️ This method utilizes sequence files generated by the `serp_peak` function within RiboParser.
