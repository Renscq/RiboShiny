...
draw_volcano <- function(
  ...
  degs_data <- degs %>% 
    dplyr::select(!!sym(label_col), 
                  ...
                  !!sym(pvalue_column), 
                  !!sym(class)) %>% 
    rename_with(~ c('Gene', 'log2FC', 'Padj', 'DEGs'), everything()) %>% 
  ...
  if (label_type == "text") {
    ######################################################
    ## draw the volcano
    volcano_plot <- ggplot(degs_data, aes(log2FC, -log(Padj, 10))) +
      ...
      geom_vline(xintercept = c(-log2fc, log2fc), color = 'black', linewidth = 0.4, linetype = 2) +
      geom_hline(yintercept = -log(pvalue, 10), color = 'black', linewidth = 0.4, linetype = 2) +
    ...
    if (nrow(label_col_gene) > 0) {
      volcano_plot <- volcano_plot +
        geom_text_repel(data = label_col_gene %>% unique(),
                        aes(x = log2FC, y = -log(Padj, 10), label = Gene),
    ...
    ## draw the volcano
    volcano_plot <- ggplot(degs_data, aes(log2FC, -log(Padj, 10))) +
      geom_point(color = ns_color, size = dot_size, alpha = 1) +
      theme_bw() + 
      theme(panel.grid = element_blank(),
            plot.title = element_text(size = font_size + 2, vjust = 0.5, hjust = 0.5, color = 'black'),
            plot.subtitle = element_text(size = font_size - 2, vjust = 0.5, hjust = 0.5, color = 'black'),
    ...
    if (nrow(label_col_gene) > 0) {
      ...
      volcano_plot <- volcano_plot +
        geom_point(data = label_col_gene %>% dplyr::distinct(),
                   aes(x = log2FC, y = -log(Padj, 10)),
                   size = dot_size, color = label_color)
    }
