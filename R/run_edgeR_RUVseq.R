...
run_edger_ruvseq <- function(
  ...
  # browser()
  set <- newSeqExpressionSet(as.matrix(exprs),
                             phenoData = data.frame(x = group, 
  ...
  # browser()
  if (method == "LRT") {
    ## 3.1 run first edgeR to get the empirical control genes
    design <- model.matrix(~x, data = pData(set))
    y <- DGEList(counts = round(counts(set), 0), group = group)
    ...
    empirical <- rownames(set)[which(!(rownames(set) %in% rownames(top)[1:emprical]))]
    
    set2 <- RUVg(set, empirical, k = 1)
    
    ## 3.2 run the edgeR with RUVseq
    design <- model.matrix(~x + W_1, data = pData(set2))
    y <- DGEList(counts = round(counts(set2), 0), group = group)
    y <- estimateGLMCommonDisp(y, design)
